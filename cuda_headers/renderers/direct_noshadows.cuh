...
__device__
vec3f tempsampleLightSource(SurfaceInteraction si, int lightIdx, float lightSelectionPdf, vec2f rand, bool mis)
{
    ...
    if (si.wo_local.z > 0.f && wi_local.z > 0.f && light_pdf > 0.f && owl::dot(-wi, lnormal) > 0.f) {
...
__device__
vec3f tempsampleBRDF(SurfaceInteraction si, float lightSelectionPdf, vec2f rand, bool mis)
{
    ...
    if (wi_local.z > 0.f && si.wo_local.z > 0.f) {
...
vec3f estimateDirectUnshadowedLighting(SurfaceInteraction& si, LCGRand& rng, int type)
{
    ...
    for (int i = 0; i < SAMPLES; i++) {
        ...
        if (type == 0) {
            ...
            lightSample = tempsampleLightSource(si, selectedTriLight, lightSelectionPdf, rand1, false);

            tmpColor = lightSample;
        }
        else if (type == 1) {
            brdfSample = tempsampleBRDF(si, 0.f, rand2, false);
            ...
            brdfSample = tempsampleBRDF(si, lightSelectionPdf, rand1, true);
            lightSample = tempsampleLightSource(si, selectedTriLight, lightSelectionPdf, rand2, true);
